...
from tomato_recon.data.pheno4d import preprocess_pheno4d
from tomato_recon.data.progress import ProgressPrinter


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert annotated Pheno4D tomato scans into canonical dataset instances"
    )
    ...
    report = preprocess_pheno4d(cfg, progress=ProgressPrinter())
    print(
        json.dumps(
            {
